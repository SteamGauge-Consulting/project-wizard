...
// Claude Fable 5, falling back to Claude Opus 5 — see lib/claude-model.js.
const models = require('./claude-model');
const MODEL = models.MODEL;
const FALLBACK_MODEL = models.FALLBACK_MODEL;
...
// Accepts one dir or an array of dirs (e.g. uploads + the GitHub repo cache).
// Every *.zip under dir (nested too — a folder upload can carry one), skipping
// the dependency/build dirs walk() ignores anyway. Bounded depth: an upload tree
// is shallow, and an unbounded walk would be a lot of statting for nothing.
function findZips(dir) {
  ...
  (function scan(rel, depth) {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) { if (!SKIP_DIR.has(e.name)) scan(rp, depth + 1); }
      else if (e.isFile() && /\.zip$/i.test(e.name) && !e.name.startsWith('._')) out.push(rp);
    }
  })('', 0);
  return out;
}

function gatherRoots(attachDirs) {
  const dirs = (Array.isArray(attachDirs) ? attachDirs : [attachDirs]).filter(Boolean);
  const roots = [];
  const temps = [];
  for (const attachDir of dirs) {
    roots.push(attachDir);
    let zips = [];
    try { zips = findZips(attachDir); } catch {}
    for (const z of zips) {
      let tmp = null;
      try {
        tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'pwcorpus-'));
        // unzip exits 1 on WARNINGS (mac resource forks, extra bytes) but still extracts,
        // so don't trust its exit code — extract, then keep the temp if anything landed.
        try { execFileSync('unzip', ['-qq', '-o', path.join(attachDir, z), '-d', tmp], { stdio: 'ignore' }); } catch (e) { /* warnings → check below */ }
        if (fs.readdirSync(tmp).length) { roots.push(tmp); temps.push(tmp); }
        else { try { fs.rmSync(tmp, { recursive: true, force: true }); } catch (e2) {} }
      } catch (e) { if (tmp) { try { fs.rmSync(tmp, { recursive: true, force: true }); } catch (e2) {} } }
    }
  }
  return { roots, temps };
}
...
// attachDir may be an array of dirs — e.g. uploads + the GitHub repo cache.
...
// AGENTIC access to the upload: instead of bulk-loading every file, expose the file
// LIST + on-demand read/search so a planning agent pulls only the files it needs.
// Keeps the extracted files on disk for the life of the index (call cleanup() after).
function buildFileIndex(attachDir) {
  const { roots, temps } = gatherRoots(attachDir);
  const raw = [];
  for (const r of roots) walk(r, '', raw);
  const seen = new Set();
  const files = [];
  for (const f of raw) {
    if (seen.has(f.rel)) continue;
    seen.add(f.rel);
    if (SKIP_FILE.test(f.rel) || isMigrationArtifact(f.rel)) continue;
    const n = f.name.toLowerCase();
    if (!(isTextual(f.name) || /\.(pdf|docx)$/.test(n))) continue;   // readable only
    let size = 0; try { size = fs.statSync(path.join(f.root, f.rel)).size; } catch {}
    files.push({ rel: f.rel, abs: path.join(f.root, f.rel), name: f.name, size });
  }
  files.sort((a, b) => score(b.rel) - score(a.rel) || a.rel.localeCompare(b.rel)); // important-first manifest
  const find = (p) => {
    const q = String(p || '').trim().toLowerCase().replace(/^\.?\//, '');
    return files.find((f) => f.rel.toLowerCase() === q)
      || files.find((f) => f.rel.toLowerCase().endsWith('/' + q))
      || files.find((f) => f.rel.toLowerCase().endsWith(q));
  };
  const READ_CAP = 110 * 1024;   // a single read_file returns at most ~28K tokens
  return {
    fileCount: files.length,
    manifest() {
      return files.map((f) => f.rel + '  (' + (f.size < 1024 ? f.size + 'B' : Math.round(f.size / 1024) + 'KB') + ')').join('\n');
    },
    list(glob) {
      let fs2 = files;
      if (glob) { const g = String(glob).toLowerCase(); fs2 = files.filter((f) => f.rel.toLowerCase().includes(g)); }
      if (!fs2.length) return '(no files match "' + glob + '")';
      return fs2.slice(0, 600).map((f) => f.rel + ' (' + Math.round(f.size / 1024) + 'KB)').join('\n');
    },
    read(p) {
      const f = find(p);
      if (!f) return 'File not found: "' + p + '". Use list_files to see exact paths.';
      const c = readContent(f.abs, f.name, f.size, f.rel);   // text / pdf / docx / data-sample
      if (!c) return '(' + f.rel + ' is empty or not readable as text)';
      return c.length > READ_CAP ? c.slice(0, READ_CAP) + '\n…[truncated at ' + Math.round(READ_CAP / 1024) + 'KB — read a more specific file or search instead]' : c;
    },
    search(pattern, max) {
      max = Math.min(max || 50, 120);
      let re; try { re = new RegExp(pattern, 'i'); } catch (e) { re = new RegExp(String(pattern).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i'); }
      const hits = [];
      for (const f of files) {
        if (hits.length >= max) break;
        if (f.size > 1024 * 1024) continue;
        let txt = ''; try { txt = fs.readFileSync(f.abs, 'utf-8'); } catch (e) { continue; }
        const lines = txt.split('\n');
        for (let i = 0; i < lines.length && hits.length < max; i++) {
          if (re.test(lines[i])) hits.push(f.rel + ':' + (i + 1) + ':  ' + lines[i].trim().slice(0, 200));
        }
      }
      return hits.length ? hits.join('\n') : '(no matches for /' + pattern + '/i)';
    },
    cleanup() { for (const t of temps) { try { fs.rmSync(t, { recursive: true, force: true }); } catch {} } },
  };
}
...
// One structured-output request against a given model. Throws with .status set;
// callers decide whether that status is worth re-running on the fallback model.
async function requestIntake(model, content, apiKey) {
  const body = {
    model: model,
    max_tokens: 16000,
    system: SYSTEM,
    // No `thinking` field: Fable 5 always thinks and rejects any explicit thinking
    // config (including "disabled"). Depth is steered by effort alone.
    output_config: { format: { type: 'json_schema', schema: INTAKE_SCHEMA }, effort: 'medium' },
    messages: [{ role: 'user', content: content }],
  };

  const ctrl = new AbortController();
  // Fable 5 thinks on every turn, so a large corpus legitimately takes minutes —
  // the old 4-minute cap cut those off. Matches the enrich/assess passes.
  const timer = setTimeout(() => ctrl.abort(), Number(process.env.WIZARD_IMPORT_TIMEOUT_MS) || 600000);
  let res;
  try {
    ...
    if (e.name === 'AbortError') {
      // 504 is deliberately NOT a fallback status: the wait has already been spent,
      // and re-running on the other model would double it with the user waiting.
      const err = new Error('the analysis timed out — try a smaller upload'); err.status = 504; throw err;
    }
    const err = new Error('could not reach the Claude API: ' + e.message); err.status = 503; throw err;
  ...
  // A safety classifier can decline the request outright — HTTP 200 with an empty
  // (or partial) content array, so check this before reading the blocks.
...
async function generateIntake(projectName, corpus, apiKey) {
  ...
  // MAX_CORPUS_CHARS is a coarse collection bound; dense corpora tokenize at
  // ~2 chars/token, so count the real tokens and trim the corpus to fit the
  // window. Required lazily to keep this module dependency-light on other paths.
  const { fitCorpus } = require('./enrich');
  const content = await fitCorpus(corpus, SYSTEM,
    function (c) { return buildUserContent(projectName, c); }, apiKey, 'reverse-engineer');

  const data = await models.withFallback('import', function (m) { return requestIntake(m, content, apiKey); });
...
module.exports = { buildCorpus, buildFileIndex, generateIntake, handoffPrompt, serverKeyConfigured, INTAKE_SCHEMA, MODEL, FALLBACK_MODEL };
