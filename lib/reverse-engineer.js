...
const MAX_CORPUS_CHARS = Number(process.env.WIZARD_CORPUS_CHARS || 600 * 1024); // ~150K tokens
const PER_FILE_BYTES = 80 * 1024;
...
const SKIP_DIR = new Set(['node_modules','.git','dist','build','.next','.nuxt','out','vendor','venv','.venv','env','__pycache__','.cache','coverage','.svelte-kit','target','.idea','.vscode','.terraform','tmp','.turbo']);
const SKIP_FILE = /(package-lock\.json|yarn\.lock|pnpm-lock\.yaml|poetry\.lock|Cargo\.lock|composer\.lock|\.min\.(js|css)$|\.map$|\.lock$)/i;
...
// Importance heuristic — read the files most likely to reveal product intent first.
function score(rel) {
  ...
  if (/(^|\/)(src|app|server|api|lib|routes|pages|components|models|controllers)\//.test(r)) s += 40;
  if (/(schema|model|migration|route|controller|handler|endpoint)/.test(r)) s += 25;
  if (/(config|settings)/.test(r)) s += 10;
...
function buildCorpus(attachDir) {
  ...
  for (const f of files) {
    ...
    seen.add(f.rel);
    let size = 0;
    try { size = fs.statSync(path.join(f.root, f.rel)).size; } catch {}
    let included = false;
    if (isTextual(f.name) && !SKIP_FILE.test(f.rel) && size <= PER_FILE_BYTES && budget > 0) {
      let content = '';
      try { content = fs.readFileSync(path.join(f.root, f.rel), 'utf-8'); } catch { content = ''; }
      if (content) {
    ...
    } else if (size > PER_FILE_BYTES || budget <= 0) {
      truncated = true;
    }
