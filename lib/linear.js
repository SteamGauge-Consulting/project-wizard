...
function cleanMs(name) { return String(name || '').replace(/^\s*M\d+\s*[·:.\-–]*\s*/i, '').trim() || String(name || '').trim(); }
...
async function createProjectWithIssues(key, opts) {
  ...
  if (!plan) {
    ...
    plan = intakeMs.length
      ? intakeMs.map((m, i) => ({ milestone: m.name, issues: reqs.filter((_, j) => Math.min(j, intakeMs.length - 1) === i).map((r) => ({ title: r.title, detail: r.test, label: DEFAULT_GOV, owner: 'AI' })) }))
      : [{ milestone: null, issues: reqs.map((r) => ({ title: r.title, detail: r.test, label: DEFAULT_GOV, owner: 'AI' })) }];
  }

  const labels = await ensureLabels(key, teamId, plan.map((_, i) => phaseName(i)));
  ...
  // milestones (with target dates) → name → { id }
  const msByName = {};
  ...
  // Phases: one parent issue per plan entry ("Phase A — …"), tagged with its
  // Phase label + the governance phases its tasks span; build tasks are
  // gov + owner-labeled sub-issues. No priority is set (everything stays
  // "No priority" rather than defaulting to Urgent).
  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    const phaseId = lid(labels, phaseName(i));
    ...
    const parent = await createIssue(key, Object.assign({
      teamId, projectId: project.id, title: phaseTitle,
      description: 'Phase parent issue. Build tasks are linked as sub-issues — start them in order.',
      labelIds: [phaseId].concat(govIds.length ? govIds : [govId(labels, DEFAULT_GOV)]).filter(Boolean),
    ...
    for (const it of issues) {
      ...
      const ids = [phaseId, govId(labels, it.label || DEFAULT_GOV), lid(labels, owner)].filter(Boolean);
      const created = await createWithKickoff(key, Object.assign({
        ...
        labelIds: ids,
      }, ms ? { projectMilestoneId: ms.id } : {}), richBody(it));
      if (created) {
        subs++;
...
async function loadProject(key, projectId) {
  const d = await gql(key,
    'query($id:String!){ project(id:$id){ name teams(first:1){ nodes{ id } } issues(first:250){ nodes{ id identifier title state{ name type } } } } }',
    { id: projectId });
...
async function loadProjectStructure(key, projectId) {
  const d = await gql(key,
    ...
    'identifier title url sortOrder assignee{ name } parent{ identifier } projectMilestone{ name } ' +
    'labels{ nodes{ name } } } } } }',
    { id: projectId });
  const proj = d.project;
  if (!proj) { const e = new Error('Linear project not found'); e.status = 404; throw e; }
  ...
  // Owner from the ASSIGNEE: a person owns it → Human; unassigned → the AI agent.
  const ownerOf = (n) => (n.assignee && n.assignee.name) ? 'Human' : 'AI';
  const govOf = (n) => labelsOf(n).find((x) => /^gov:/.test(x)) || DEFAULT_GOV;
  const bySort = (a, b) => (a.sortOrder || 0) - (b.sortOrder || 0);
  // Parents = issues that have at least one sub-issue, in board order.
  const parents = nodes.filter((n) => childrenOf[n.identifier]).sort(bySort);
  let subs = 0; const msSet = {};
  const phases = parents.map((par, i) => {
    const kids = (childrenOf[par.identifier] || []).slice().sort(bySort);
    const ms = (par.projectMilestone && par.projectMilestone.name)
      || (kids[0] && kids[0].projectMilestone && kids[0].projectMilestone.name) || null;
    if (ms) msSet[ms] = 1;
    const issues = kids.map((k) => { subs++; return { title: k.title, identifier: k.identifier, url: k.url, owner: ownerOf(k), label: govOf(k) }; });
