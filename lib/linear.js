...
async function loadProjectStructure(key, projectId) {
  // Paginate — Linear caps a page at 250 issues and this loader must see the
  // whole project or milestones silently lose rows.
  const QUERY = 'query($id:String!,$after:String){ project(id:$id){ name url ' +
    'projectMilestones(first:50){ nodes{ name description targetDate sortOrder } } ' +
    'issues(first:250, after:$after){ pageInfo{ hasNextPage endCursor } nodes{ ' +
    'identifier title url sortOrder parent{ identifier } projectMilestone{ name } ' +
    'labels{ nodes{ name } } } } } }';
  let proj = null; const nodes = []; let after = null;
  for (let page = 0; page < 4; page++) {
    const d = await gql(key, QUERY, { id: projectId, after });
    const pr = d.project;
    if (!pr) { const e = new Error('Linear project not found'); e.status = 404; throw e; }
    proj = proj || pr;
    nodes.push(...((pr.issues && pr.issues.nodes) || []));
    const pi = (pr.issues && pr.issues.pageInfo) || {};
  ...
  return { url: proj.url, projectId: projectId,
    counts: { milestones: milestones.length, phases: milestones.length, issues: subs, totalIssues: nodes.length },
    milestones: milestones };
}

// Propagate intake milestone edits to the tracker: pair the before/after rows
// (by position), and for each changed row rename the matching Linear project
// milestone — preserving any "Phase X — " prefix — plus every top-level parent
// issue titled with it, and refresh targetDate/description when target or
// done-means changed. Keeps tracker names from drifting away from the docs.
async function syncMilestoneEdits(key, projectId, before, after, startDate) {
  const out = { renamed: [], parents: 0, skipped: [] };
  const pairs = [];
  const n = Math.min((before || []).length, (after || []).length);
  for (let i = 0; i < n; i++) {
    const a = before[i] || {}, z = after[i] || {};
    if (cleanMs(a.name) !== cleanMs(z.name) || String(a.target || '') !== String(z.target || '') || String(a.done || '') !== String(z.done || '')) pairs.push([a, z]);
  }
  if (!pairs.length) return out;
  const d = await gql(key,
    'query($id:String!){ project(id:$id){ projectMilestones(first:50){ nodes{ id name } } ' +
    'issues(first:250){ nodes{ id title parent{ identifier } } } } }', { id: projectId });
  const proj = d.project;
  if (!proj) { const e = new Error('Linear project not found'); e.status = 404; throw e; }
  const mss = (proj.projectMilestones && proj.projectMilestones.nodes) || [];
  const issues = ((proj.issues && proj.issues.nodes) || []).filter((x) => !(x.parent && x.parent.identifier));
  const lc = (s) => cleanMs(s).toLowerCase();
  for (const [a, z] of pairs) {
    const oldClean = cleanMs(a.name), newClean = cleanMs(z.name);
    const ms = mss.find((m) => lc(m.name) === oldClean.toLowerCase());
    if (!ms) { out.skipped.push(a.name || '(unnamed)'); continue; }
    const prefix = (String(ms.name).match(/^\s*Phase\s+[A-Z]+\s*[—\-–]\s*/) || [''])[0];
    const newName = (prefix + newClean).slice(0, 80);
    const targetDate = weekTargetDate(startDate, z.target);
    const desc = [z.done ? 'Done means: ' + z.done : '', z.target ? 'Target: ' + z.target : ''].filter(Boolean).join('\n');
    await gql(key, 'mutation($id:String!,$input:ProjectMilestoneUpdateInput!){ projectMilestoneUpdate(id:$id, input:$input){ success } }',
      { id: ms.id, input: Object.assign({ name: newName, description: desc || undefined }, targetDate ? { targetDate } : {}) });
    out.renamed.push({ from: ms.name, to: newName });
    if (oldClean.toLowerCase() !== newClean.toLowerCase()) {
      for (const it of issues) {
        const m = String(it.title || '').match(/^(\s*Phase\s+[A-Z]+\s*[—\-–]\s*)(.+)$/);
        if (!m || cleanMs(m[2]).toLowerCase() !== oldClean.toLowerCase()) continue;
        await gql(key, 'mutation($id:String!,$input:IssueUpdateInput!){ issueUpdate(id:$id, input:$input){ success } }',
          { id: it.id, input: { title: (m[1] + newClean).slice(0, 250) } });
        out.parents++;
      }
    }
  }
