...
async function loadProjectStructure(key, projectId) {
  ...
  const toIssue = (k) => { subs++; return { title: String(k.title || '').replace(/🔑️?/g, '').replace(/\s{2,}/g, ' ').trim(), identifier: k.identifier, url: k.url, owner: ownerOf(k), label: govOf(k) }; };
  const parents = nodes.filter((n) => childrenOf[n.identifier]).sort(byTitle);
  let subs = 0; const msSet = {};
  let phases = parents.map((par, i) => {
    ...
    return { letter: phaseLetter(i), name: par.title, milestone: ms ? cleanMs(ms) : null,
      parent: { title: par.title, identifier: par.identifier, url: par.url }, issues: kids.map(toIssue) };
  });

  // Fallback: no "Phase A/B" parent hierarchy (issues sit directly under
  // milestones). Group EVERY issue by its milestone so the Plan still shows them
  // all. These carry no letter/parent, so render-intake renders the issues
  // straight under each milestone (no phase sub-head).
  if (phases.length === 0 && nodes.length) {
    const byMs = {};
    nodes.forEach((n) => { const ms = (n.projectMilestone && n.projectMilestone.name) || ''; (byMs[ms] = byMs[ms] || []).push(n); });
    const order = Object.keys(byMs).sort((a, b) => (a === '' ? 1 : b === '' ? -1 : String(a).localeCompare(String(b), undefined, { numeric: true, sensitivity: 'base' })));
    phases = order.map((ms) => {
      if (ms) msSet[ms] = 1;
      return { letter: '', name: ms || 'Unscheduled', milestone: ms ? cleanMs(ms) : null, parent: null, issues: byMs[ms].slice().sort(byTitle).map(toIssue) };
    });
  }
