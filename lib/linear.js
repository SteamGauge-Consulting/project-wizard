...
async function createProjectWithIssues(key, opts) {
  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    for (const it of issues) {
      const ids = [phaseId, govId(labels, it.label || DEFAULT_GOV), lid(labels, it.owner === 'Human' ? 'Human' : 'AI')].filter(Boolean);
  ...
  return { url: project.url, counts: { milestones: Object.keys(msByName).length, phases: parents, issues: subs } };
}
