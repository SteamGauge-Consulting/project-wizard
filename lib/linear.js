...
async function loadProjectStructure(key, projectId) {
  ...
  // Phases are TOP-LEVEL parents only: an issue with children but no parent of its
  // own. A sub-issue that grows its own sub-sub-issues stays INSIDE its phase (its
  // subtree flattens depth-first below it, with depth for indentation) instead of
  // being promoted to a bogus extra "phase". Only "Phase …"-titled parents count —
  // other top-level groupings in the tracker stay off the executive plan.
  const parents = nodes.filter((n) => childrenOf[n.identifier] && !(n.parent && n.parent.identifier) && /^\s*phase\b/i.test(n.title || '')).sort(byTitle);
  // Trust the letter already in the parent's title ("Phase B — …") over the
  // positional fallback, so display letters keep matching the tracker's naming
  // even if extra top-level groupings sort in between.
  const letterOf = (title, i) => { const m = /^\s*Phase\s+([A-Z]+)\b/.exec(String(title || '')); return m ? m[1] : phaseLetter(i); };
  const descendants = (id, depth, out) => {
    (childrenOf[id] || []).slice().sort(byTitle).forEach((k) => { out.push({ node: k, depth }); descendants(k.identifier, depth + 1, out); });
    return out;
  };
  let subs = 0; const msSet = {};
  const phases = parents.map((par, i) => {
    const kids = descendants(par.identifier, 0, []);
    const ms = (par.projectMilestone && par.projectMilestone.name)
      || (kids[0] && kids[0].node.projectMilestone && kids[0].node.projectMilestone.name) || null;
    if (ms) msSet[ms] = 1;
    const issues = kids.map((kd) => {
      const k = kd.node;
      subs++;
      ...
      return { title: String(k.title || '').replace(/🔑️?/g, '').replace(/\s{2,}/g, ' ').trim(),
        identifier: k.identifier, url: k.url, owner: ownerOf(k), label: govOf(k), depth: kd.depth };
    });
    return { letter: letterOf(par.title, i), name: par.title, milestone: ms ? cleanMs(ms) : null,
