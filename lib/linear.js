...
async function createProjectWithIssues(key, opts) {
  ...
  const msByName = {};
  let msOrder = 0;   // explicit ascending sortOrder so M1…Mn list in order, not reversed
  for (const m of intakeMs) {
    ...
    const r = await gql(key, 'mutation($input: ProjectMilestoneCreateInput!){ projectMilestoneCreate(input:$input){ success projectMilestone{ id } } }',
      { input: Object.assign({ name: (m.name || 'Milestone').slice(0, 80), projectId: project.id, sortOrder: msOrder++, description: desc || undefined }, targetDate ? { targetDate } : {}) });
  ...
  let parents = 0, subs = 0, issueOrder = 0;   // issueOrder → explicit ascending sortOrder (build order, not reversed)
