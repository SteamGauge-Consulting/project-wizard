...
function render(genDir, intake, opts) {
  ...
  rewriteNav(genDir, { name, docsDir, adrs, ms, domain, wizardEditUrl: opts.wizardEditUrl || '' });
...
function rewriteNav(genDir, ctx) {
  ...
  // Point "← App" at the product's real app (its domain), not the docs pod root.
  if (ctx.domain) {
    const appHref = /^https?:\/\//.test(ctx.domain) ? ctx.domain : 'https://' + ctx.domain;
    src = src.replace(/const APP\s*=\s*"(?:[^"\\]|\\.)*";/,
      'const APP     = ' + JSON.stringify('<a href="' + appHref + '" style="color:#7A7A75;text-decoration:none;padding:5px 9px;border-radius:7px;margin-left:auto;">← App</a>') + ';');
  }
