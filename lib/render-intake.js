...
function render(genDir, intake, opts) {
  ...
  const GANTT_LIVE = gmMap.length
    ? '<script>window.__gmMap=' + JSON.stringify(gmMap) + ';</script>\n<script>\n' +
      ...
      "  const clean = (x) => { x = String(x||'').trim(); if (x.slice(0,6) !== 'Phase ') return x; var seps = [' — ',' – ',' - ']; for (var k=0;k<seps.length;k++){ var i=x.indexOf(seps[k]); if(i>=0) return x.slice(i+seps[k].length).trim(); } return x; };\n" +
      '  const byClean = {}; Object.keys(s.milestones).forEach((k) => { byClean[clean(k)] = s.milestones[k]; });\n' +
      '  const done = (n) => { const m = s.milestones[n] || byClean[clean(n)]; return m && m.total > 0 && m.done >= m.total; };\n' +
      '  let tries = 0;\n' +
      '  const paint = () => {\n' +
      "    const rects = document.querySelectorAll('.gantt-wrap rect.task');\n" +
      '    if (!rects.length) { if (tries++ < 50) return setTimeout(paint, 150); return; }\n' +
      "    map.forEach((e, i) => { if (!done(e.ms)) return; const r = document.querySelector('.gantt-wrap rect[id$=\"-' + e.id + '\"]') || document.getElementById(e.id) || rects[i]; if (r) { r.style.fill = '#33611f'; r.style.stroke = '#97C459'; } });\n" +
      '  };\n  paint();\n} catch (e) {} })();\n</script>' : '';
  ...
  if (hasLive) {
    ...
    const ovHtml = groups.map((g) => {
      ...
      const phasesHtml = g.phases.map((ph) => {
        // Flat (milestone-grouped) phases carry no letter/parent — render their
        // issues straight under the milestone, no "Phase X" sub-head.
        const flat = !ph.parent && !ph.letter;
