...
function htmlShell(title, intro, bodyHtml) {
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
function render(genDir, intake, opts) {
  ...
  for (const a of adrs) {
    writeFile(path.join(root, 'adr', a.file), [
      '# ADR-' + a.n + ' · ' + a.concern, '',
      '- **Status:** Accepted', '',
      '## Decision', '', a.choice || '_(not specified)_', '',
      '## Why / trade-off accepted', '', a.why || '_(not specified)_', '',
      '*A coding agent will expand this into full Context · Decision · Trade-off · Revisit-trigger per the governance library.*', '',
    ].join('\n'));
  }
  ...
  writeFile(path.join(root, 'plan.html'), htmlShell(name + ' — build plan', 'Milestones, requirements, and risks from the intake.', planBody));
  ...
  writeFile(path.join(root, 'architecture.html'), htmlShell(name + ' — architecture', 'The locked stack and non-functional posture.', archBody));
