...
(async () => {
  try {
    ...
    for (let t = 0; t < 3 && !(s && s.ok); t++) {
      ...
      try { s = await (await fetch('/docs/api/status')).json(); } catch (e) {}
    }
    if (!s || !s.ok) return;
    const by = {};
...
function render(genDir, intake, opts) {
  ...
  const stakeholders = clean(intake.stakeholders, ['name', 'email', 'phone', 'role']);
  ...
  // Stakeholders — its own tab: who is involved, their role, and how to reach
  // them. Always written (empty state included) so nav / routes / check-docs
  // stay consistent on intakes that predate the section.
  const stakeBody =
    '<h2>Stakeholders</h2>' +
    (stakeholders.length ? '<div class="panel"><table><thead><tr><th>Name</th><th>Role in project</th><th>Email</th><th>Phone</th></tr></thead><tbody>' +
      stakeholders.map((s) => {
        const tel = String(s.phone || '').replace(/[^\d+]/g, '');   // digit-free "phones" render as plain text, not a dead tel: link
        return '<tr><td><b>' + esc(s.name) + '</b></td><td>' + esc(s.role) + '</td>' +
          '<td>' + (String(s.email || '').trim() ? '<a href="mailto:' + escA(s.email) + '">' + esc(s.email) + '</a>' : '<span class="pill todo">—</span>') + '</td>' +
          '<td>' + (tel ? '<a href="tel:' + escA(tel) + '">' + esc(s.phone) + '</a>'
            : (String(s.phone || '').trim() ? esc(s.phone) : '<span class="pill todo">—</span>')) + '</td></tr>';
      }).join('') +
      '</tbody></table></div>'
      : '<p class="lead">No stakeholders recorded yet — add them on the Stakeholders step of the wizard.</p>') +
    '<p class="meta" style="margin-top:14px">See also <a href="/docs/plan">Plan</a> · <a href="/docs/requirements">Requirements</a>.</p>';
  writeFile(path.join(root, 'stakeholders.html'), htmlShell(name + ' — stakeholders', 'Who is involved in this project and how to reach them.', stakeBody));
  ...
  const buildBody =
    ...
    '<h2>Resources</h2><ul class="res">' +
      ...
      // (No WORKFLOW.md link — the donor's workflow file is project-specific
      // and deleted in §1; nothing regenerates it, so linking it breaks check-docs.)
...
function rewriteNav(genDir, ctx) {
  ...
  // Match a proper JS string literal (escaped-pair aware), not a lazy "[^]*?";
  // — on pods the fallback re-patches an ALREADY-REWRITTEN file, and a lazy
  // match terminates at an embedded \"; (e.g. an ADR concern containing ";),
  // truncating mid-string and corrupting docs-nav.js.
  ...
  src = src.replace(/const TOP = \[[\s\S]*?\];/,
    ...
    "  { key: 'stakeholders', href: '/docs/stakeholders', label: 'Stakeholders' },\n" +
    "];");
...
function rewriteRoutes(genDir, ctx) {
  ...
  if (!/\/docs\/stakeholders'/.test(src)) {
    const row = "  { path: '/docs/stakeholders',    file: '" + d + "/stakeholders.html', navKey: 'stakeholders', serves: 'Stakeholders — who is involved and how to reach them' },\n";
    src = src.replace(/(\{ path: '\/docs\/requirements',[^}]*\},\n)/, '$1' + row);
  }
