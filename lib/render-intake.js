...
function render(genDir, intake, opts) {
  ...
  // Build — mirrors the reference /docs/build: the tools we code with, how work
  // is tracked, the per-issue process, and resources. All derived from the intake.
  const trackerName = integ.tracker || 'Linear';
  ...
  const coreVendors = decs.filter((d) => !SIDE_RE.test(d.concern || '')).slice(0, 5).map(vendorOf).filter(Boolean).join(' · ') || 'your stack';
  const stackSummary = decs.slice(0, 6).map((d) => d.choice || d.concern).filter(Boolean).join(' · ');
  ...
  const PHASES = [['01-requirements', '01 · Requirements & planning'], ['02-architecture', '02 · Architecture'], ['03-design', '03 · Design'], ['04-implementation', '04 · Implementation'], ['05-review', '05 · Review'], ['06-testing', '06 · Testing'], ['07-release', '07 · Release']];
  const BUILD_PAGE_CSS = '<style>' +
    'ol.proc{list-style:none;padding:0;display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:10px;margin:14px 0}' +
    'ol.proc li{background:var(--panel);border:1px solid var(--line);border-radius:9px;padding:11px 14px;font-size:14px}' +
    'ol.proc li a{color:var(--text);text-decoration:none}ol.proc li a:hover{color:var(--accent)}' +
    'ul.res{list-style:none;padding:0;margin:12px 0}ul.res li{padding:9px 0;border-bottom:1px solid var(--line);font-size:14px;color:var(--muted)}' +
    'ul.res li a{color:var(--accent);text-decoration:none;font-weight:500}ul.res li a:hover{text-decoration:underline}' +
    '</style>';
  const buildBody =
    '<h2>What we code with</h2><div class="grid">' +
      '<div class="card"><div class="k">AI builder</div><h3>Claude Code</h3><p>Writes the code in the repo under the governance phases. Every commit is prefixed <code>[agent:claude-code]</code>.</p></div>' +
      '<div class="card"><div class="k">Independent review</div><h3>Cross-examiner</h3><p>A second agent independently reviews every PR before it can merge — the Phase 05 review checklist + acceptance-criteria trace.</p></div>' +
      '<div class="card"><div class="k">Discipline</div><h3>Governance library</h3><p>7 phase guides (requirements → release) plus the global laws. <a href="/docs/claude-init">See the framework →</a></p></div>' +
      '<div class="card"><div class="k">Source control</div><h3>GitHub</h3><p>Repo, PRs, branch protection, and the cross-examiner CI gate. <code>main</code> requires a green review to merge.</p></div>' +
      '<div class="card"><div class="k">Production stack</div><h3>' + esc(coreVendors) + '</h3><p>' + (stackSummary ? esc(stackSummary) : 'The locked production stack') + '. Decisions live in the <a href="/docs/architecture">ADRs</a>.</p></div>' +
      (sideTitles.length ? '<div class="card"><div class="k">Side services</div><h3>' + esc(sideTitles.slice(0, 4).join(' · ')) + '</h3><p>Supporting services around the core app — email, error tracking, and off-site backups.</p></div>' : '') +
    '</div>' +
    '<h2>How we track work</h2><div class="grid">' +
      '<div class="card"><div class="k">Tracker</div><h3>' + esc(trackerName) + '</h3><p>' + (ms.length ? ms.length + ' milestones' : 'Milestones') + ' → phases → atomic sub-issues' + (totalSubs ? ' · ' + totalSubs + ' issues' : '') + '. Each issue&rsquo;s steps are checkboxes you tick as you go.' + (trackerUrl ? ' <a href="' + escA(trackerUrl) + '" target="_blank" rel="noopener">Open the project →</a>' : '') + '</p></div>' +
      '<div class="card"><div class="k">Overview</div><h3>The plan</h3><p>What every issue delivers + live status, for a high-level read. <a href="/docs/plan">See the plan →</a></p></div>' +
    '</div>' +
    '<h2>The per-issue process</h2>' +
    '<p class="lead">Every issue moves through the seven governance phases — requirements to release — each with a checklist guide:</p>' +
    '<ol class="proc">' + PHASES.map((p) => '<li><a href="/docs/library/' + p[0] + '">' + esc(p[1]) + '</a></li>').join('') + '</ol>' +
    '<p class="lead">Pick a requirement from the <a href="/docs/plan">plan</a>, work it through the phases, and ship. The <a href="/docs/requirements">requirements</a> doc is the contract.</p>' +
    '<h2>Resources</h2><ul class="res">' +
      '<li><a href="/docs/view/AGENTS.md">AGENTS.md</a> — cross-agent contract, read first</li>' +
      '<li><a href="/docs/claude-init">Framework</a> — the governance framework at a glance</li>' +
      '<li><a href="/docs/view/governance/CLAUDE.md">Governance overview</a> — which phase guide to load</li>' +
      '<li><a href="/docs/requirements">Requirements</a> — what we&rsquo;re building (and not)</li>' +
      '<li><a href="/docs/architecture">Architecture</a> — the build bible</li>' +
      '<li><a href="/docs/view/' + docsDir + '/adr/README.md">ADR index</a> — the locked decisions</li>' +
    '</ul>';
  writeFile(path.join(root, 'build.html'), htmlShell(name + ' — build', 'Everything a developer needs to pick up a ' + esc(trackerName) + ' issue and ship it the way we work — the tools we code with, where work is tracked, and the per-issue process.', buildBody, BUILD_PAGE_CSS));
