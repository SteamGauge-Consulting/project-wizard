...
const githubCorpus = require('./github-corpus');
...
module.exports = function mountEditor(app, opts) {
  ...
  function githubCfg() {
    const k = readKeys();
    const token = String(k.githubToken || process.env.GITHUB_TOKEN || '').trim();
    return { repoUrl: String(k.githubRepoUrl || process.env.GITHUB_REPO_URL || '').trim(), token: token, hasToken: !!token };
  }
  ...
  // Corpus roots for Assess: the bundled reference/ uploads PLUS (when a GitHub
  // repo is configured — Integrations tab or deploy-baked env) a cached zipball
  // of the repo's latest commit, so assess reads the CURRENT code rather than
  // whatever was frozen into the deploy. The cache lives in .deploy/github-cache
  // (bind-mounted, survives restarts; redeploys exclude it). Fail-soft: GitHub
  // trouble never blocks assess when reference/ (or the cache) has code.
  function githubCacheDir() { return path.join(deployDir(), 'github-cache'); }
  async function corpusRoots() {
    const roots = [referenceDir()];
    const gh = githubCfg();
    const github = { configured: !!gh.repoUrl, sha: null, stale: false, error: null };
    if (!gh.repoUrl) return { roots: roots, github: github };
    try {
      const r = await githubCorpus.syncRepoCache({ repoUrl: gh.repoUrl, token: gh.token, cacheDir: githubCacheDir() });
      roots.push(r.dir);
      github.sha = r.sha;
      github.stale = !!r.stale;
      if (r.warning) github.error = r.warning;
    } catch (e) { github.error = e.message || String(e); }
    return { roots: roots, github: github };
  }
  ...
  app.get('/api/intake', function (req, res) {
    ...
    // Sync path — count the cached repo zip as corpus, but don't hit GitHub
    // here; a configured repo URL alone promises a corpus (assess fetches it).
    let corpus = null;
    try { corpus = reverse.buildCorpus([referenceDir(), path.join(githubCacheDir(), 'zip')]); } catch (e) {}
    res.json({
      ok: true,
      intake: intake,
      hasClaude: !!anthropicKey(),
      hasLinear: !!(linearProjectId() && linearKey()),
      hasCorpus: !!(corpus && corpus.includedCount) || !!githubCfg().repoUrl,
  ...
  // ── what Linear structure the DOCS were last rendered with ──────────────────
  // Persisted (.deploy/linear-shape.json) so the background sync can tell "the
  // docs already show this structure" from "the docs never got a live-Linear
  // render" — the latter happened whenever the deploy-time rerender missed its
  // window or its Linear fetch failed, and the old first-tick-is-baseline sync
  // then never repaired it (the TRACKER hadn't changed, only the docs were
  // stale), forcing a manual ☰ → Sync from Linear.
  function linearShapeOf(lr) {
    return JSON.stringify(((lr && lr.milestones) || []).map((m) =>
      [m.name, m.targetDate, m.doneMeans, (m.issues || []).map((i) => i.identifier + '|' + i.title + '|' + i.depth)]));
  }
  let lastShape = null;   // in-memory mirror of the shape file
  function readRenderedShape() {
    try { return String(fs.readFileSync(path.join(deployDir(), 'linear-shape.json'), 'utf-8')); }
    catch (e) { return null; }
  }
  function noteRenderedLinear(lr) {
    lastShape = linearShapeOf(lr);
    try { fs.writeFileSync(path.join(deployDir(), 'linear-shape.json'), lastShape); } catch (e) {}
  }
  ...
  app.post('/api/rerender', jsonBody, async function (req, res) {
    ...
    try {
      ...
      if (lr) noteRenderedLinear(lr);
      res.json({ ok: true, synced: !!lr });
    } catch (e) { res.status(500).json({ error: 'render failed: ' + (e.message || e) }); }
  });

  // ── Linear → pod live sync ────────────────────────────────────────────────
  // Poll the tracker and re-render in place when it differs from what the docs
  // were LAST RENDERED WITH (not merely from the previous poll), so the project
  // can be managed in Linear day-to-day and these docs follow — no wizard
  // round-trip. Comparing against the rendered state (persisted across
  // restarts) also makes the first tick self-healing: a pod whose docs never
  // got a live-Linear render pulls one automatically instead of waiting for a
  // manual ☰ → Sync from Linear.
  ...
  async function linearSyncTick() {
    ...
    try {
      ...
      if (lastShape === null) lastShape = readRenderedShape();
      if (lastShape === linearShapeOf(lr)) return;   // docs already show this structure
      let intake;
      ...
      try { enrich = JSON.parse(fs.readFileSync(path.join(deployDir(), 'enrich.json'), 'utf-8')); } catch (e) {}
      renderIntake.render(ROOT, intake, { docsDir: docsDirOf(intake), enrich: enrich, linear: lr });
      noteRenderedLinear(lr);
      console.log('linear-sync: docs re-rendered with the live tracker structure');
  ...
  app.post('/api/assess', jsonBody, async function (req, res) {
    ...
    try { const cr = await corpusRoots(); ghc = cr.github; corpus = reverse.buildCorpus(cr.roots); } catch (e) {}
    if (!corpus || !corpus.includedCount) {
      return res.status(400).json({ error: 'No code for Assess to read', code: 'no_corpus',
        detail: 'Assess analyzes code impact against the real source. Re-deploy this project with a code/zip upload in reference/, or set the GitHub repo URL (+ token for private repos) on the Integrations tab so assess pulls the latest code itself.' + (ghc.error ? ' GitHub: ' + ghc.error : '') });
    }
  ...
  app.post('/api/apply', jsonBody, async function (req, res) {
    ...
    try { renderIntake.render(ROOT, merged, { docsDir: docsDir, enrich: enrich, linear: lrApply }); if (lrApply) noteRenderedLinear(lrApply); }
