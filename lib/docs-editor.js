  ...
  app.get('/api/status', async function (req, res) {
    ...
    try {
      const r = await fetch('https://api.linear.app/graphql', {
        ...
        body: JSON.stringify({
          query: 'query($id:String!){ project(id:$id){ name issues(first:250){ nodes{ identifier title sortOrder state{ name type } parent{ identifier } projectMilestone{ name } } } } }',
          variables: { id: pid },
        }),
      });
      const json = await r.json().catch(() => null);
      const proj = json && json.data && json.data.project;
      const nodes = proj && proj.issues && proj.issues.nodes;
      ...
      const issues = nodes.map((n) => ({
        id: n.identifier, title: n.title, sortOrder: n.sortOrder,
      ...
      // Roll-ups (overall + per-milestone → home tiles, phase cards, gantt
      // painting) count ONLY descendants of "Phase …"-titled top-level parents —
      // the same executive-plan filter the Plan page uses — so stray tracker
      // groupings can't skew the numbers. The full issues list still ships for
      // per-pill painting.
      const kidsOf = {};
      issues.forEach((i) => { if (i.parentId) (kidsOf[i.parentId] = kidsOf[i.parentId] || []).push(i); });
      const planIds = new Set();
      const walk = (id) => (kidsOf[id] || []).forEach((k) => { planIds.add(k.id); walk(k.id); });
      issues.filter((i) => !i.parentId && kidsOf[i.id] && /^\s*phase\b/i.test(i.title || '')).forEach((r) => walk(r.id));
      const subs = issues.filter((i) => planIds.has(i.id));
      const milestones = {};
      subs.forEach((i) => {
