...
module.exports = function mountEditor(app, opts) {
  ...
  const LINEAR_PROJECT_ID = (opts.linearProjectId || process.env.LINEAR_PROJECT_ID || '').trim();
  ...
  app.get('/api/intake', function (req, res) {
    ...
    res.json({
      ...
      hasLinear: !!(LINEAR_PROJECT_ID && linearKey()),
  ...
  app.post('/api/assess', jsonBody, async function (req, res) {
    ...
    const lk = linearKey();
    let linearIssues = [];
    if (LINEAR_PROJECT_ID && lk) {
      try { const meta = await linear.loadProject(lk, LINEAR_PROJECT_ID); linearIssues = meta.issues; }
    ...
    res.json({
      ...
      hasLinear: !!(LINEAR_PROJECT_ID && lk),
  ...
  app.post('/api/apply', jsonBody, async function (req, res) {
    ...
    // 2. Re-render the docs in place from the merged intake. Re-enrich with the
    //    baked Claude key (fresh diagrams + acceptance criteria), else plain
    //    tables. The live /api/status script overlays Linear state, so a null
    //    linear structure here is fine — the Plan page stays live.
    const docsDir = docsDirOf(merged);
    const apiKey = anthropicKey();
    let enrich = null;
    if (applied.docSections.length && apiKey) {
      try {
        let corpus = null; try { corpus = reverse.buildCorpus(referenceDir()); } catch (e) {}
        enrich = await enrichLib.enrich(merged, apiKey, corpus && corpus.includedCount ? corpus : null);
    ...
    const lk = linearKey();
    if (LINEAR_PROJECT_ID && lk && acceptedUnits.some((u) => u.group === 'linear' || u.group === 'affected-closed')) {
      let meta = null;
      try { meta = await linear.loadProject(lk, LINEAR_PROJECT_ID); } catch (e) { errors.push('Linear load: ' + e.message); }
      if (meta) {
        const byId = {}; meta.issues.forEach((i) => { byId[i.identifier] = i; });
        for (const u of acceptedUnits) {
          try {
            if (u.group === 'linear' && u.action === 'create') {
              const issue = await linear.createIssueForChange(lk, { teamId: meta.teamId, projectId: LINEAR_PROJECT_ID, title: u.title, owner: u.owner, label: u.label, objective: u.objective, reason: u.reason, changeId: changeId });
              ...
              if (u.action === 'update') { await linear.updateIssue(lk, issue, changeId, u.objective || u.reason); applied.linear.push({ action: 'update', identifier: issue.identifier }); }
              else { await linear.cancelIssue(lk, issue, meta.states, changeId, u.objective || u.reason); applied.linear.push({ action: 'cancel', identifier: issue.identifier }); }
