...
function mount(app, deps) {
  const { storage, intakeOf, summarize, emptyAnswers, walkTree, safeGenPath, PORT, rerenderGenerated } = deps;
  ...
  app.delete('/api/projects/:id/agent-tokens/:tid', (req, res) => {
    const p = storage.getProject(req.params.id);
    if (!p) return res.status(404).json({ error: 'not found' });
    if (!tokens.deleteToken(p, req.params.tid)) return res.status(404).json({ error: 'token not found' });
    storage.saveProject(p);
    res.json({ ok: true });
  });

  // Re-open the paste-in kit for a live key (owner-side; the retained plaintext
  // token never leaves via any agent route). ?base= bakes the LAN URL in.
  app.get('/api/projects/:id/agent-tokens/:tid/kit', (req, res) => {
    const p = storage.getProject(req.params.id);
    if (!p) return res.status(404).json({ error: 'not found' });
    const rec = (p.agentTokens || []).find((t) => t.id === req.params.tid);
    if (!rec) return res.status(404).json({ error: 'token not found' });
    if (rec.revokedAt) return res.status(410).json({ error: 'this key has been revoked' });
    if (!rec.token) return res.status(409).json({ error: 'this key predates prompt re-display — revoke it and create a new one to get a shareable prompt', code: 'no_secret' });
    const base = kitBase(req, deps, req.query.base);
    res.json({ ok: true, token: rec.token, scope: rec.scope, name: rec.name, base, kit: tokens.kit(p, rec.token, rec.scope, base) });
  });
  ...
  // Content edits also re-render the generated /docs tree — the tree is a RENDER
  // of the answers and deploys ship it verbatim, so skipping this left stale
  // ADR/requirement markdown in the bundle after agent edits. Connection-cred
  // changes and lifecycle forwards don't touch doc content (the forwarded
  ...
  function commit(req, op, detail) {
    ...
    storage.saveProject(p);
    if (rerenderGenerated && op !== 'patch:connections' && !/^lifecycle:/.test(op)) rerenderGenerated(p);
  }
  ...
  r.get('/connections', async (req, res) => {
    ...
    // Pull the deployed pod's live Integrations-tab keys (its keys.json — where
    // the org actually enters Claude/Linear/GitHub creds) so the agent sees them
    // with no re-entry. Falls back to per-project overrides + wizard env.
    let podKeys = null, keysFromPod = false;
    const dep = req.pwProject.deployUrl;
    if (dep) {
      try {
        const origin = String(dep).replace(/\/docs\/?$/, '');
        const mh = {};
        const mtEnv = String(process.env.PW_MACHINE_TOKEN || '').trim();
        let mt = mtEnv;
        if (!mt) { try { mt = require('fs').readFileSync(require('path').join(deps.dataRoot || '', 'machine-token'), 'utf8').trim(); } catch (e) {} }
        if (mt) mh['x-pw-machine'] = mt;
        const rr = await fetch(origin + '/api/integrations/keys.json', { headers: mh, signal: AbortSignal.timeout(6000) });
        if (rr.ok) { podKeys = await rr.json(); keysFromPod = true; }
      } catch (e) { /* pod unreachable — fall back to overrides/env */ }
    }
    const { effective, configured } = tokens.connectionsView(req.pwProject, process.env, write, podKeys);
    res.json({
      ok: true, scope: req.pwToken.scope, includesSecrets: write,
      effective, configured, keysFromPod,
      deployUrl: dep || null, deployedAt: req.pwProject.deployedAt || null,
      hint: write
        ? 'Live credentials for this project' + (keysFromPod ? ' — Claude/Linear/GitHub read from the deployed pod’s Integrations tab' : '') + '. SSH to sshHost as sshUser on sshPort (sshPassword if set, else the wizard’s mounted key). POST /deploy reuses these when omitted.'
        : 'Secret values are withheld from read-only tokens — `configured` shows which are set. Mint a write-scope token to retrieve the actual keys.',
    });
  });
