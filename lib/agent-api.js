...
function mount(app, deps) {
  ...
  // ── UI-side integration keys (project.connections) — what an agent reads via
  //    /connections. Status only (never secret values); secrets kept unless a new
  //    non-empty value is supplied, non-secret fields set as given.
  app.get('/api/projects/:id/connections', (req, res) => {
    const p = storage.getProject(req.params.id);
    if (!p) return res.status(404).json({ error: 'not found' });
    const view = tokens.connectionsView(p, process.env, false);
    res.json({ effective: view.effective, configured: view.configured, fields: tokens.CONNECTION_FIELDS, secretFields: Array.from(tokens.SECRET_FIELDS) });
  });
  app.post('/api/projects/:id/connections', (req, res) => {
    const p = storage.getProject(req.params.id);
    if (!p) return res.status(404).json({ error: 'not found' });
    const b = req.body || {};
    const c = Object.assign({}, p.connections || {});
    tokens.SECRET_FIELDS.forEach((k) => {
      if (b[k] === null) delete c[k];                                     // explicit null clears
      else if (typeof b[k] === 'string' && b[k].trim()) c[k] = b[k].trim(); // blank keeps existing
    });
    tokens.CONNECTION_FIELDS.filter((k) => !tokens.SECRET_FIELDS.has(k)).forEach((k) => {
      if (typeof b[k] === 'string') { const v = b[k].trim(); if (v) c[k] = v; else delete c[k]; } // non-secret set/clear as given
    });
    p.connections = c;
    storage.saveProject(p);
    const view = tokens.connectionsView(p, process.env, false);
    res.json({ ok: true, effective: view.effective, configured: view.configured });
  });
