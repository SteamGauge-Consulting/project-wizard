...
const ASSESS_SCHEMA = obj({
  ...
  sectionImpacts: arr(obj({
    section: { type: 'string', enum: ['product', 'requirements', 'decisions', 'milestones', 'risks', 'scalability', 'stakeholders', 'startDate'], description: 'Which intake section this note is about' },
...
const rowLabel = { requirements: 'title', decisions: 'concern', milestones: 'name', risks: 'risk', scalability: 'area', stakeholders: 'name' };
...
function buildUserContent({ baseline, proposed, docDiff, linearIssues, corpus }) {
  const slim = (it) => ({
    ...
    stakeholders: (it.stakeholders || []).map((s) => ({ name: s.name, email: s.email, phone: s.phone, role: s.role })),
  });
...
// claude-model decides which of those are worth re-running on the fallback.
async function requestAssessment(model, content, apiKey) {
  const body = {
    model: model,
    max_tokens: 32000,
    system: SYSTEM,
  ...
  // Assess sends the code corpus + live tracker and the model thinks throughout;
  // allow 10 min (tunable).
  ...
  try {
    ...
    // 504 is not a fallback status — a spent timeout must not be doubled.
    if (e.name === 'AbortError') { const err = new Error('the assessment timed out'); err.status = 504; throw err; }
  ...
  // A safety classifier can decline outright — HTTP 200, empty content — so check
  // this before reading the blocks.
...
async function assess(input, apiKey) {
  ...
  // The corpus char cap is a coarse collection bound, not a token guarantee —
  // dense corpora tokenize at ~2 chars/token and can overflow the 1M window.
  // fitCorpus counts the real tokens and trims ONLY the corpus to fit; the
  // baseline/proposed/diff/tracker sections always survive intact.
  const content = await fitCorpus(input.corpus, SYSTEM,
    function (c) { return buildUserContent(Object.assign({}, input, { corpus: c })); }, apiKey, 'assess');

  const data = await models.withFallback('assess', function (m) { return requestAssessment(m, content, apiKey); });
