...
function files(opts) {
  ...
  const routerLabels = routes.map((r, i) => {
    ...
    // Both entrypoints: the host's Traefik terminates TLS on `websecure` with
    // a wildcard *.<ip>.nip.io default cert, so a pod is reachable over https
    // the moment it starts — no per-pod cert labels. http stays live too (the
    // wizard↔pod deploy/rerender callbacks POST over http://).
    return '      - traefik.http.routers.' + rn + '.rule=' + rule + '\n' +
           '      - traefik.http.routers.' + rn + '.entrypoints=web,websecure\n' +
  ...
  const githubRepoUrl = (opts.githubRepoUrl || '').trim(); // lets the pod's Assess pull the repo's LATEST code
  const githubToken = (opts.githubToken || '').trim();     // needed only for private repos
  ...
  if (githubRepoUrl) envEntries.push('GITHUB_REPO_URL=' + githubRepoUrl);
  if (githubToken) envEntries.push('GITHUB_TOKEN=' + githubToken);
  ...
  // The shared machine token — lets the wizard↔pod deploy/rerender callbacks pass
  // each other's Entra gate. Same value on both, owned by the wizard.
  if (opts.machineToken) envEntries.push('PW_MACHINE_TOKEN=' + String(opts.machineToken).trim());
  // Entra SSO config for the pod (only when the wizard forwards it, i.e. this
  // deployment should be gated). Reproducible per-domain: each pod carries its
  // own tenant/client/allow-list; the redirect URI is this pod's public origin.
  const ent = opts.entra || {};
  if (String(ent.mode || '').toLowerCase() === 'entra' && ent.tenant && ent.clientId && ent.clientSecret) {
    envEntries.push('AUTH_MODE=entra');
    envEntries.push('ENTRA_TENANT_ID=' + String(ent.tenant).trim());
    envEntries.push('ENTRA_CLIENT_ID=' + String(ent.clientId).trim());
    envEntries.push('ENTRA_CLIENT_SECRET=' + String(ent.clientSecret).trim());
    if (ent.allowed) envEntries.push('ENTRA_ALLOWED=' + String(ent.allowed).trim());
    // The pod's public origin for the OIDC redirect URI — must match the Azure
    // redirect URI. Prefer an explicit override, else the app-domain route
    // mounted at a path (the real public sign-in surface, e.g.
    // crm.treebrandapps.com/docs) over a bare LAN nip.io route, else host:port.
    let pub = ent.publicUrl ? String(ent.publicUrl).trim() : '';
    if (!pub) {
      const authRoute = routes.find((r) => r.indexOf('/') !== -1) || routes[0] || '';
      const pubHost = authRoute ? authRoute.split('/')[0] : (host + ':' + port);
      const pubPath = authRoute && authRoute.indexOf('/') !== -1 ? ('/' + authRoute.slice(authRoute.indexOf('/') + 1).replace(/\/+$/, '')) : '';
      // LAN hosts (embedded IPv4, or nip.io/sslip.io wildcard DNS) are plain http;
      // real domains front TLS at the proxy → https.
      const isLan = /\d+\.\d+\.\d+\.\d+/.test(pubHost) || /\.(nip|sslip)\.io$/i.test(pubHost) || !/\./.test(pubHost);
      const proto = isLan ? 'http' : 'https';
      pub = proto + '://' + pubHost + pubPath;
    }
    envEntries.push('AUTH_PUBLIC_URL=' + pub);
  }
...
analyzes your edits against the code in \`reference/\` — plus the linked GitHub
repo's latest commit, when a repo URL is configured — and the live Linear tracker,
