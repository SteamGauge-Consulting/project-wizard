...
const COLLECTIONS = {
  ...
  stakeholders: { keys: ['name', 'email', 'phone', 'role'], label: 'stakeholder' },
};
...
// process.env; pass it in so the module stays pure/testable. `podKeys` is the
// deployed pod's live Integrations-tab keys (its /api/integrations/keys.json) —
// where the org actually enters Claude/Linear/GitHub creds. Precedence:
// per-project agent override → pod Integrations tab → wizard env → derived.
function resolveConnections(project, env, podKeys) {
  env = env || {};
  ...
  return {
    sshHost:         firstNonEmpty(c.sshHost, pk.sshHost, dt.host, env.DEPLOY_SSH_HOST, derivedHost),
    sshUser:         firstNonEmpty(c.sshUser, pk.sshUser, dt.user, env.DEPLOY_SSH_USER, 'docker'),
    sshPort:         firstNonEmpty(c.sshPort, pk.sshPort, dt.sshPort, env.DEPLOY_SSH_PORT, '22'),
    sshPassword:     firstNonEmpty(c.sshPassword, pk.sshPassword, env.DEPLOY_SSH_PASSWORD),
    hostname:        firstNonEmpty(c.hostname, dt.hostname),
    anthropicKey:    firstNonEmpty(c.anthropicKey, pk.anthropicKey, env.ANTHROPIC_API_KEY),
    grokKey:         firstNonEmpty(c.grokKey, pk.grokKey, env.GROK_API_KEY, env.XAI_API_KEY),
    linearKey:       firstNonEmpty(c.linearKey, pk.linearKey, env.LINEAR_API_KEY),
    linearProjectId: firstNonEmpty(c.linearProjectId, pk.linearProjectId, project && project.linearProjectId, integ.linearProjectId, env.LINEAR_PROJECT_ID),
    githubRepoUrl:   firstNonEmpty(c.githubRepoUrl, pk.githubRepoUrl, integ.githubRepoUrl, env.GITHUB_REPO_URL),
    githubToken:     firstNonEmpty(c.githubToken, pk.githubToken, env.GITHUB_TOKEN),
  };
}
...
function connectionsView(project, env, includeSecrets, podKeys) {
  const r = resolveConnections(project, env, podKeys);
...
const OPS = [
  ...
  { method: 'GET',    path: '/<collection>',       scope: 'read',  summary: 'List the rows of a collection (' + Object.keys(COLLECTIONS).join(' | ') + ').' },
