...
function revokeToken(project, tokenId) {
  const t = (project.agentTokens || []).find((x) => x.id === tokenId);
  if (!t || t.revokedAt) return false;
  t.revokedAt = new Date().toISOString();
  return true;
}

// Drop the secret hash before a token record ever leaves the server.
function redactRecord(t) {
  return { id: t.id, name: t.name, scope: t.scope, prefix: t.prefix, createdAt: t.createdAt, lastUsedAt: t.lastUsedAt || null, revokedAt: t.revokedAt || null };
}
...
module.exports = {
  ...
  parseToken, mintToken, findRecord, revokeToken, redactRecord, redactProject, publicProject,
