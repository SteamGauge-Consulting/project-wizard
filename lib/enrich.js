...
async function enrichPlan(intake, apiKey, corpus) {
  apiKey = (apiKey && String(apiKey).trim()) || process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return { plan: [], usage: [] };
  ...
  // Cohesion across the separate per-milestone calls: each call sees the FULL intake
  // (all milestones/requirements/decisions/code) AND the titles of issues already
  // created in earlier milestones — so it won't duplicate them, can build on them,
  // and the seam between milestones stays clean.
  const callFor = async (m, priorTitles) => {
    const priorBlock = priorTitles.length
      ? '\n\nAlready created in EARLIER milestones (do NOT duplicate these — assume they exist, build on them, reference them where relevant):\n' +
        priorTitles.map((t) => '• ' + t).join('\n')
      : '';
    const focus = '\n\n=== YOUR TASK ===\nGenerate ONLY the atomic sub-issues for the milestone "' + m.name + '"' +
      (m.done ? ' (done means: ' + m.done + ')' : '') + '.\n' +
      'Cover its full scope: every requirement it implies, plus the phase tasks that belong to it ' +
      '(setup, schema/migrations, each feature, integration, tests, review, release — whichever apply). ' +
      'Be exhaustive and granular — do NOT hold back detail to save space; you have a full budget for this one milestone. ' +
      'Do NOT include work that belongs to the other milestones (they are handled separately).' + priorBlock;
    const body = {
      ...
      messages: [{ role: 'user', content: buildUserContent(intake, corpus) + focus }],
    };
    const data = await claudeFetch(apiKey, body);
  ...
  if (!milestones.length) {
    ...
    const text = ((data && data.content) || []).filter((b) => b.type === 'text').map((b) => b.text).join('');
    let parsed = {}; try { parsed = JSON.parse(text); } catch (e) {}
    return { plan: Array.isArray(parsed.plan) ? parsed.plan : [], usage };
  }

  const plan = [];
  const priorTitles = [];   // accumulates across milestones for cohesion
  for (const m of milestones) {
    const issues = await callFor(m, priorTitles);
    plan.push({ milestone: m.name, issues });
    issues.forEach((it) => { if (it && it.title) priorTitles.push(m.name + ': ' + it.title); });
  }
  return { plan, usage };
}
