...
async function enrichPlan(intake, apiKey, corpus) {
  ...
  const callFor = async (m) => {
    const focus = '\n\n=== YOUR TASK ===\nGenerate ONLY the atomic sub-issues for the milestone "' + m.name + '"' +
      ...
      'Do NOT include work that belongs to the other milestones (they are handled separately).';
    const body = {
  ...
  const plan = [];
  for (const m of milestones) plan.push({ milestone: m.name, issues: await callFor(m) });
  return { plan, usage };
}
