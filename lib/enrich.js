...
// The build plan is generated by its OWN focused AI call (enrichPlan) so it gets a
// full token budget — otherwise it starves at the end of the big docs enrichment
// and comes out coarse (one issue per milestone, empty steps, everything AI).
const PLAN_PROP = arr(obj({
  ...
  }, ['title', 'owner', 'objective', 'steps', 'notes', 'doneWhen', 'label']), 'The atomic sub-issues under this milestone, in build order — MULTIPLE per milestone (one per discrete requirement/feature/deliverable, PLUS the phase tasks: setup, schema/migration, implementation, tests, review, release). A milestone with real scope has SEVERAL issues; never collapse it into one mega-issue.'),
...
async function enrichPlan(intake, apiKey, corpus) {
  ...
  if (!apiKey) return [];
  const body = {
    model: MODEL, max_tokens: 32000, system: PLAN_SYSTEM,
  ...
  const text = ((data && data.content) || []).filter((b) => b.type === 'text').map((b) => b.text).join('');
  const parsed = JSON.parse(text);
  const raw = Array.isArray(parsed && parsed.plan) ? parsed.plan : [];
  // Merge any duplicate milestone groups the model emitted into one group each,
  // preserving order, so the tracker gets one phase per milestone.
  const byMs = new Map();
  for (const entry of raw) {
    const key = String((entry && entry.milestone) || '');
    const issues = (entry && entry.issues) || [];
    if (byMs.has(key)) byMs.get(key).issues.push(...issues);
    else byMs.set(key, { milestone: entry && entry.milestone, issues: issues.slice() });
  }
