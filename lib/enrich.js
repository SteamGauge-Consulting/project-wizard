...
const ENRICH_SCHEMA = obj({
  ...
  plan: arr(obj({
    ...
    issues: arr(obj({
      ...
      steps: arr(str('One concrete, self-contained step. HUMAN tasks: a fully click-by-click action a non-technical person can follow blind — name the exact site/screen, the button or menu by its label, what to type, the precise value to copy and exactly where to paste it, and how to confirm it worked. AI tasks: a precise, imperative build instruction — name the exact file(s)/module(s) to create or change, the functions/endpoints/data shapes involved, the approach, the libraries and commands, and the edge cases to handle.'), 'The COMPLETE ordered step-by-step — exhaustive enough to execute start to finish without asking a question or re-deriving anything. Use as many steps as the work genuinely needs (typically 6–14); never collapse real work into one vague line.'),
      notes: str('Deep execution detail beyond the steps. AI tasks: the technical approach, every file/module/function to touch, data shapes & schemas, libraries (with versions where it matters), commands to run, how to test it, and the gotchas / failure modes to avoid — written so a coding agent produces production-quality work on the FIRST pass. HUMAN tasks: what to have ready beforehand, direct links, account/permission prerequisites, and common pitfalls. Markdown allowed.'),
...
const SYSTEM =
  ...
  'These tasks become Linear issues a person or a coding agent will execute, so make each one an EXHAUSTIVE, self-contained runbook entry — detailed enough that the assignee never has to ask a question or re-derive the plan. Never a one-line stub. For EACH task give: the owner ("Human" for work only a person can do — signups, minting API keys, DNS/billing/account setup, approvals, manual verification — or "AI" for code/docs/tests a coding agent can do); a one-sentence objective; the COMPLETE ordered steps; deep execution notes; a done-when acceptance check; and the single best governance label (requirements → architecture → design → implementation → review → testing → release; owner-action for human-only setup).\n\n' +
  'Tailor the depth to the owner — err on the side of MORE detail, always:\n' +
  '• HUMAN tasks → the ENTIRE manual workflow, click by click, written for someone who has never used the tool: the exact site/screen, the button or menu by name, what to type, the precise value to copy and exactly where to paste it, and how to confirm each step worked. Leave nothing implicit.\n' +
  '• AI tasks → treat the issue like a mini-spec that lets Claude do its best work in ONE pass: name every file/module/function to create or change, the data shapes & schemas, the endpoints, the libraries (and versions where it matters), the exact commands, the tests to write, and the edge cases / failure modes to handle. Concrete and imperative, grounded in the locked decisions and the uploaded code.\n\n' +
...
module.exports = { enrich, enrichArchitecture, aiEnabled, ENRICH_SCHEMA, MODEL, GOV_LABELS };
