...
async function enrichArchitecture(intake, apiKey, corpus) {
  ...
  const body = {
    ...
    system: 'You are a senior software architect. Produce ONLY the system architecture as structured layer data (the `architecture` field) plus a one-line note, exactly per the field descriptions — technically precise, grounded in the locked decisions and the real codebase, and CONCISE like a clean reference diagram (the people get one line, no bullets). The top "User" lane holds the end User + Developer + Claude Code; lower lanes are the category layers; draw the dev pipeline + runtime flows as edges.',
