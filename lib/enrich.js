...
const ENRICH_SCHEMA = obj({
  architectureMermaid: str('A Mermaid `flowchart TB` modeling BOTH the production system AND the development pipeline, drawn as compact stacked LAYERS (top → bottom). Follow every rule: (1) TOP ROW = the people: a "User" node (the end user) and a "Developer" node as two plain nodes side by side on the top row — do NOT wrap them in a subgraph box (that leaves an ugly gap); just declare them first so they share the top rank. (2) Below them, one `subgraph` per major architectural LAYER, stacked top→bottom, EACH with `direction LR` so its nodes form a horizontal band. Pick the layers the locked decisions imply — typically "Web / Edge" (CDN, DNS, marketing), "Application" (the runtime/API we build), "Data & Services" (databases, auth, billing, email, other third-party SaaS), and "CI/CD & Deploy". (3) Show the DEVELOPMENT loop explicitly and visually: Developer → Claude Code → GitHub → CI → deploy → the Application; label those edges (e.g. "prompts", "push", "CI", "deploy"). (4) Show the RUNTIME path: User → Web/Edge → Application → Data & Services, with labeled edges (HTTPS, JWT verify, SQL, webhook, …). (5) Be AS COMPACT AS POSSIBLE without losing accuracy or readability: only the layers/nodes the architecture truly needs, merge closely-related services into one node where sensible, keep labels short. (6) Give every node a TWO-line <br/> label: the name on the first line, then ONE short PLAIN-ENGLISH line saying what it does for the product, written so a project manager or executive with only mild technical experience understands it — NO ADR numbers, NO phase labels, NO deep jargon (write "Where the app runs" not "Stateless container (ADR-005)"; "Stores everyone\'s data securely" not "Postgres with row-level security"). (7) Assign EVERY node exactly one style class via `class <id> <class>` using ONLY: actor (the User and the Developer), edge (CDN/DNS/proxy/static), app (OUR application/runtime/API), saas (third-party SaaS — incl. Claude Code), data (databases/caches/stores), infra (GitHub, CI, registry, deploy target). Do NOT emit `classDef` or `style` lines — the renderer supplies the palette. Output valid Mermaid only: no code fences, no markdown.'),
...
const SYSTEM =
  'You expand a software project plan into richer documentation and a governance-phased build runbook. You are given the project intake (product, requirements, decisions, milestones) and, when available, excerpts of the existing/related codebase. Produce: a Mermaid architecture diagram grounded in the locked decisions — a COMPACT, layered `flowchart TB` with the User and the Developer on the top row, stacked category layers (Web/Edge, Application, Data & Services, CI/CD), the development loop drawn explicitly (Developer → Claude Code → GitHub → CI → deploy → app) alongside the runtime path, multi-line node labels, one style class per node, and labeled edges, exactly as the architectureMermaid field describes; a Mermaid Gantt from the milestones; one Given/When/Then acceptance criterion per requirement (concrete and observable — an actor and an outcome, never implementation detail); a full ADR body per decision (context, trade-off accepted, revisit trigger); and a `plan` that breaks the work into the intake milestones, each with build tasks (sub-issues) in build order.\n\n' +
...
const ARCH_RULES =
  'A compact Mermaid `flowchart TB` drawn as stacked LAYERS, top → bottom. ' +
  '(1) Top row = the people: a "User" node (end user) and a "Developer" node, plain nodes side by side, NOT wrapped in a subgraph box. ' +
  '(2) Below them, one `subgraph` per major layer, each with `direction LR` so its nodes form a horizontal band — e.g. "Web / Edge", "Application", "Data & Services", "Build & Deploy". ' +
  '(3) Show the dev loop: Developer → Claude Code → GitHub → deploy → the Application, with labeled edges. ' +
  '(4) Show the runtime path: User → Web/Edge → Application → Data & Services, with labeled edges. ' +
  '(5) Every node label is TWO lines via <br/>: the name, then ONE short plain-English line a project manager or executive with mild tech experience understands — NO ADR numbers, NO phase labels, NO jargon. ' +
  '(6) Every node gets exactly one style class via `class <id> <class>` from: actor (User, Developer), edge, app, saas (incl. Claude Code), data, infra (GitHub/CI/deploy). ' +
  '(7) As compact, balanced, and uncrossed as possible. No `classDef`/`style` lines. Valid Mermaid only — no code fences, no prose.';
